...
impl XuguConnectOptions {
    pub(crate) fn parse_from_url(url: &Url) -> Result<Self, Error> {
        ...
        if !user.is_empty() {
            options = options.user(&percent_decode_str(user).decode_utf8()?);
        }
        ...
        if !path.is_empty() {
            options = options.database(&percent_decode_str(path).decode_utf8()?);
        }
        ...
        for (key, value) in url.query_pairs().into_iter() {
            ...
            match &*key {
                ...
                "version" => options = options.version(value.parse()?),
