...
use rbdc::io::{BufStream, Encode};
use rbdc::net::Socket;
use rbdc::rt::AsyncReadExt;
...
impl AsyncStreamExt for XuguStream {
    ...
    async fn read_bytes(&mut self, len: usize) -> Result<Bytes> {
        let mut buf = self.socket.read_raw(len).await?;
        if self.use_ssl {
    ...
    async fn handshake_recv(&mut self, opts_version: i16) -> Result<bool> {
        ...
        match cmd {
            ...
            b'E' | b'F' => {
                ...
                return Err(XuguDatabaseError::from_str(&err_msg).into());
            }
            _ => (),
        }
...
impl XuguStream {
    ...
    fn write_packet<'en, T>(&mut self, payload: T)
    where
        T: Encode<'en, ()>,
    {
        let s = self.socket.wbuf.len();
        self.socket.write_with(payload, ());
        let e = self.socket.wbuf.len();

        if self.use_ssl {
            let buf = &mut self.socket.wbuf[s..e];
